...
class MedicalSuppliesPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.keyword = ""
        self._TABLE_NAME = "medical_supplies"
    ...
    def delete_selected(self):
        ...
        query = f"DELETE FROM {self._TABLE_NAME} WHERE medical_item_id = %s;"
        try:
            self.cursor.executemany(query, selected_ids)
            self.controller.connection.commit()
            self.search_records()
            self.controller.add_log(f"Deleted the following item ids from {self._TABLE_NAME} table: {selected_ids}")
    ...
    def search_records(self):
        ...
        try:
            query = f"""
                ...
                WHERE medical_item_id LIKE %s
                    OR name LIKE %s
                    OR quantity LIKE %s
                ORDER BY medical_item_id
            """
            ...
            # Checking if any Medical Supplies matching keyword were found
            if len(rows) == 0:
                messagebox.showinfo("No Items Found",
                                    f"Could not find any medical supplies matching {self.keyword}")
                return
    ...
    def load_records(self):
        try:
            ...
            messagebox.showerror(
                "Database Error",
                f"Could not Load Medical Supply Records. \n\n{e}"
            )

    def open_create_supply_popup(self):
        create_supply_popup = CreateMedicalSupplyPopUp(self.controller, self)
        self.wait_window(create_supply_popup)
    ...
    def _create_widgets(self):
        ...
        create_btn = globals.create_styled_button(
            ...
            text="Add Medical Supply",
            command=self.open_create_supply_popup,
            style="BTN.TButton"
        )
        create_btn.grid(
            row=0,
...
class MedicalSupplyDetailWindow(tk.Toplevel):
    ...
    def get_entry_data(self) -> dict:
        ...
        output["medical_item_id"] = self.item_data.get("medical_item_id")
        return output
    ...
    def save_update(self):
        ...
        if form_data.get("medical_item_id") is None:
        ...
        if diff > 0:
            ...
            qty_log_str = f"decreased by {abs(diff)} ({old_quantity} -> {new_quantity})"
        else:
            qty_log_str = "remained unchanged"
        ...
        query = """
            ...
            SET
                ...
                quantity = %(quantity)s
            WHERE 
                medical_item_id = %(medical_item_id)s
        """
        try:
            ...
            messagebox.showinfo("Updated Successfully!",
                                f"Updated '{form_data.get('name')}' successfully!\nQuantity {qty_log_str}.")
            self.parent.search_records()
            self.destroy()
        except mysql.connector.Error as e:
            self.controller.connection.rollback()
            messagebox.showerror("ERROR! Could not Update Record",
                                 f"An error occurred while trying to update '{form_data.get('name')}'"
                                 f"\n\nError Message: {e}")


class CreateMedicalSupplyPopUp(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        ttk.Button(
            button_frame,
            text="Submit",
            command=self.add_supply_record,
            style="BTN.TButton",
            cursor="hand2"
        ).grid(
            row=0,
    ...
    def add_supply_record(self):
        ...
        # Validate Quantity field is an integer
        try:
            int(data["QUANTITY"])
        except ValueError:
            messagebox.showerror("Invalid Quantity", "Quantity must be a valid whole number.")
            return

        query = """
                INSERT INTO medical_supplies (name, quantity)
                VALUES (%(NAME)s, %(QUANTITY)s);
                """
        try:
            ...
            self.controller.connection.commit()
            self.controller.add_log(f"Added medical supply item '{data['NAME']}' with quantity {data['QUANTITY']}.")
            messagebox.showinfo(
                "Success",
                "Medical supply record has been added successfully."
            )
            self.parent.search_records()
            self.close_window()
